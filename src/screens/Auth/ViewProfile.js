...
const { View, Text, Platform, TouchableHighlight,Alert } = require("react-native");
...
import { EDIT_PROFILE_FIELDS, VIEW_PROFILE_FIELDS, LOGGOUT_BTN_ID } from "./constants";
...
class ViewProfile extends Component {
  ...
  actionTrigger({ action }) {
    ...
    const actions = {logout:{
      action:()=>this._logout(),label:'Are you sure you want to logout?'
    }}
  ...
  render() {
    ...
    return (
      (isEdit && (
        <EditProfile userId={userId} onDone={() => this.onDoneEditing()} />
      )) || (
        <Container>
          <Content padder>
            <Card>
            ...
            <Card>
              <List>{this.renderProfile({ user })}</List>
            </Card>
          </Content>
        </Container>
      )
